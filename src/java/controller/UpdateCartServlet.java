        ...
        response.setContentType("text/html;charset=UTF-8");
         try (PrintWriter out = response.getWriter()) {
            ...
            if (cart != null && cart.size() > 0) {
                // lấy carid và quantity từ trang viewcart.jsp
                ...
                if (findCar != null) {
                    cart.put(findCar, Integer.parseInt(quantity.trim()));
                    // cập nhật cart map vào session
                    session.setAttribute("cart", cart);
                }
            }
            response.sendRedirect("MainController?action=8");
        }
    }
